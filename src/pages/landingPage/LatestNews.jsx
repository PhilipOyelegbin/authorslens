...
const LatestNews = () => {
    ...
    useEffect(() => {
        ...
        fetch(import.meta.env.VITE_API_TOP_NEWS_URL + query)
            .then(resp => resp.json())
            .then(data => data.data ? setLatestNews(data.data) : setError("Unexpected error, check back later"))
    ...
    <section id="latest-blogs" className='col-span-2'>
        ...
        <div className='grid grid-cols-1 gap-5 md:grid-cols-2 mt-5'>
            {LatestNews && LatestNews?.map(news => (
                <figure className='shadow-md shadow-slate-500 rounded-2xl ease-in-out duration-300 hover:-translate-y-2 max-w-2xl' key={news.uuid}>
